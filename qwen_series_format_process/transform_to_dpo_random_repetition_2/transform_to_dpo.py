...
def extract_css_rules_group(content: str, pos: int, num_rules: int = 3) -> Optional[str]:
    """
    Extract multiple consecutive CSS rules as a group.
    Now supports 3-5 rules for more realistic repetition.
    Returns multiple CSS rules combined, e.g.:
    ".class1 { color: red; }
     .class2 { color: blue; }
     .class3 { margin: 10px; }"
    """
...
def generate_css_multi_rule_loop(content: str, start_pos: int, target_length: int) -> str:
    ...
    Uses 3-5 consecutive rules for more realistic patterns.
    Example:
        .class1 { color: red; }
        .class2 { margin: 10px; }
        .class3 { padding: 5px; }
    ...
    # Randomly choose 3, 4, or 5 rules to repeat together
    num_rules = random.choice([3, 4, 5])
...
def generate_css_large_block_loop(content: str, start_pos: int, target_length: int) -> str:
    """
    Generate loopy content by repeating a large block of CSS (5-10 rules).
    Creates patterns like the Font Awesome example where multiple related
    CSS rules are repeated as a unit.

    Example:
        .fa-envelope { font-family: "Font Awesome"; font-weight: 400; }
        .fa-envelope-o { font-family: "Font Awesome"; font-weight: 400; }
        .fa { display: inline-block; font-size: 14px; }
        .fa-home { color: blue; }
        .fa-user { color: red; }
        .fa-envelope { font-family: "Font Awesome"; font-weight: 400; }
        .fa-envelope-o { font-family: "Font Awesome"; font-weight: 400; }
    ...
    # Randomly choose 5-10 rules for a large block
    num_rules = random.choice([5, 6, 7, 8, 9, 10])
    rules_group = extract_css_rules_group(content, start_pos, num_rules)

    if not rules_group:
        # Try with fewer rules if not enough available
        for fallback_num in [5, 4, 3, 2]:
            rules_group = extract_css_rules_group(content, start_pos, fallback_num)
            if rules_group:
                break
...
def generate_css_growing_selector_loop(content: str, start_pos: int, target_length: int) -> str:
    """
    Generate loopy content with growing CSS selectors.
    Each rule has an additional combinator element added to the selector.
    ...
    Example:
        ...
        .nav--list + ul + ul + ul + ul + ul {
            margin-top: 0
        }
    """
    prefix = content[:start_pos]

    # Try to find a full CSS selector including combinators directly from a rule
    # Pattern to match full selector with combinators before {
    full_selector_pattern = re.compile(r'([.#]?[\w-]+(?:\s*[+>~\s]\s*[.#]?[\w-]+)*)\s*\{', re.DOTALL)

    search_start = max(0, start_pos - 300)
    matches = list(full_selector_pattern.finditer(content, search_start, min(start_pos + 300, len(content))))

    base_selector = None
    combinator = None

    if matches:
        closest_match = min(matches, key=lambda m: abs(m.start() - start_pos))
        full_selector = closest_match.group(1).strip()

        # Try to find a combinator pattern in the selector (+ ul, > div, etc.)
        combinator_pattern = re.compile(r'(\s*[+>~]\s*[a-zA-Z][a-zA-Z0-9]*)')
        combinator_matches = list(combinator_pattern.finditer(full_selector))

        if combinator_matches:
            # Use the last combinator found as the repeating unit
            combinator = combinator_matches[-1].group(1)
            base_selector = full_selector
        else:
            # No combinator in selector, use simple selector with new combinator
            base_selector = full_selector
            combinator = random.choice([' + ul', ' + li', ' + div', ' > div', ' > li', ' ~ p'])
    else:
        # Fallback defaults
        base_selector = '.nav--list'
        combinator = random.choice([' + ul', ' + li', ' + div', ' > div', ' > li', ' ~ p'])

    # Try to find the CSS properties from a nearby rule
    css_rule = extract_css_rule(content, start_pos)
    if css_rule:
        # Extract just the properties part { ... }
        props_match = re.search(r'\{([^}]*)\}', css_rule)
        if props_match:
    ...
    # Generate rules with growing selectors
    # Start with base selector having a few combinators already
    current_selector = base_selector
    # Add initial combinators if the selector doesn't already have many
    combinator_count = len(re.findall(r'[+>~]', current_selector))
    if combinator_count < 3:
        for _ in range(random.randint(3, 6)):
            current_selector += combinator

    # Estimate rule length
    sample_rule = f'{current_selector} {{\n    {properties}\n}}\n'
    repeat_count = get_random_rep_count(remaining, len(sample_rule))

    suffix = ""
    for _ in range(repeat_count):
        rule = f'{current_selector} {{\n    {properties}\n}}\n'
        suffix += rule
        current_selector += combinator  # Grow the selector for next iteration
...
def generate_loopy_content(content: str) -> Tuple[str, str]:
    ...
    if is_in_css_region(content, start_pos):
        # CSS region - 8 loop types
        css_choices = ["css_rule", "css_property", "css_selector", "css_incrementing", "css_value", "css_multi_rule", "css_large_block", "css_growing_selector"]
        ...
        elif loop_type == "css_large_block":
            result = generate_css_large_block_loop(content, start_pos, target_length)
        else:  # css_growing_selector
            result = generate_css_growing_selector_loop(content, start_pos, target_length)
    else:
...
def main():
    ...
    loop_type_counts = {
        ...
        "css_multi_rule": 0,
        "css_large_block": 0,
        "css_growing_selector": 0,  # NEW - growing selectors like .nav + ul + ul + ul
    }
