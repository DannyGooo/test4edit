...
# repetition rejected samples, half get full-coverage semantic perturbation.
# Uses transform_to_dpo_infinite_nesting_no_length_match.py for repetition
# and transform_to_dpo_semantic_full_coverage.py for semantic.
#
...
#
# Usage:
#   ./run_transform_dpo_combined.sh --input <input.jsonl> --output <output.jsonl> \
...
OUTPUT="/home/liu282/scratch3/projects/vision_to_code/dataset_clean/htmlSlicer/output/screenshot_final_outout/ms_swift/meta_data_web_clean_8000_1280_12800_human_read_token8000_dpo_combined.jsonl"
LIMIT=""
SEED="42"
PERTURB_RATE="0.7"
CATEGORIES="all"
...
usage() {
    echo -e "${BLUE}ms_swift DPO Dataset Transformation Tool (Combined: Repetition + Semantic)${NC}"
    echo ""
    echo "Combines two rejection strategies into a single DPO dataset:"
    echo ""
    echo -e "${YELLOW}Repetition half:${NC}"
    echo "  - 25 failure sub-patterns from infinite-nesting generator"
    echo "  - 12 completion-failure (cf_*) + 13 inline-repetition (inline_*)"
    echo "  - No length-match guarantee (variable rejected length)"
    echo ""
    ...
    echo "  # Default 50/50 split"
    echo "  $0 -i input.jsonl -o output_dpo.jsonl"
    echo ""
    echo "  # 70% semantic, 30% repetition"
    echo "  $0 -i input.jsonl -o out.jsonl --split 0.7"
    echo ""
    echo "  # 100% repetition (equivalent to run_transform_dpo_infinite_nesting_no_length_match.sh)"
...
echo -e "${BLUE}=============================================================${NC}"
echo -e "${BLUE}ms_swift DPO Transformation (Combined: Repetition + Semantic)${NC}"
echo -e "${BLUE}=============================================================${NC}"
echo ""
...
# Build command
CMD="python3 ${SCRIPT_DIR}/transform_to_dpo_combined.py --input \"$INPUT\" --output \"$OUTPUT\" --seed $SEED --perturb-rate $PERTURB_RATE --categories \"$CATEGORIES\" --split $SPLIT"
