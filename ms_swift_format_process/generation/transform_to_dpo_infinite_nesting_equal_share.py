...
All 25 sub-patterns are length-matched: len(rejected) == len(chosen). Unlike
the sibling infinite-nesting script (which weights patterns by real-world
frequency), this variant samples ALL 25 sub-patterns with EQUAL probability
(1/25 each). Prerequisite-dependent patterns (css_*, section_repetition, …)
re-roll to another pattern if the input lacks the required structure.
...
Usage:
  python3 transform_to_dpo_infinite_nesting.py --input data.jsonl --output data_dpo.jsonl [--limit N] [--seed S]
"""
...
def generate_inline_repetition(content: str) -> Tuple[str, str]:
    """
    Inline-repetition dispatcher (length-matched).
    Picks one of 13 loop types based on whether the random start position
    falls inside a CSS region or HTML region.
    """
...
# -----------------------------------------------------------------------------
# Equal-share helpers: pick a start_pos inside / outside CSS regions so we can
# force a specific inline loop_type regardless of where the random start would
# naturally land.
# -----------------------------------------------------------------------------

def _pick_css_start_pos(content: str) -> Optional[int]:
    """Pick a random position strictly inside a <style>…</style> block."""
    regions: List[Tuple[int, int]] = []
    for m in re.finditer(r'<style[^>]*>', content, re.IGNORECASE):
        start = m.end()
        close = re.search(r'</style>', content[start:], re.IGNORECASE)
        if close:
    ...
    return random.randint(lo, max(lo, hi - 1))


def _pick_html_start_pos(content: str) -> int:
    """Pick a random position OUTSIDE any <style> block (best-effort)."""
    for _ in range(8):
        start_pct = random.uniform(0.01, 0.95)
        pos = int(len(content) * start_pct)
        if not is_in_css_region(content, pos):
            return pos
    # Fallback: just use the first valid non-CSS offset we can find.
    return int(len(content) * 0.05)


INLINE_HTML_TYPES = ["char", "tag", "section", "incrementing",
                     "closing_tag", "self_closing", "deeply_nested"]
INLINE_CSS_TYPES = ["css_rule", "css_property", "css_selector",
                    "css_incrementing", "css_value", "css_multi_rule"]


def generate_inline_repetition_forced(content: str, loop_type: str) -> Optional[Tuple[str, str]]:
    """
    Force a specific inline loop type. Returns None if the prerequisite is
    missing (e.g. caller asked for an inline_css_* pattern but the content has
    no <style> block) so the dispatcher can re-roll.
    """
    target_length = len(content)
    if loop_type in INLINE_CSS_TYPES:
        start_pos = _pick_css_start_pos(content)
        if start_pos is None:
            return None
    else:
        start_pos = _pick_html_start_pos(content)
...
def generate_css_bloat(content: str, force_low_volume: Optional[bool] = None) -> Optional[Tuple[str, str]]:
    """
    ...
    if force_low_volume is None:
        low_volume = random.random() < 0.30
    else:
        low_volume = force_low_volume
...
def generate_infinite_nesting_rejected(content: str) -> Tuple[str, str]:
    ...
    All sub-patterns are length-matched: len(rejected) == len(chosen).
    ...
    # ----- Equal-share dispatch across all 25 sub-patterns -----
    # Unlike the sibling infinite-nesting script (which weights patterns by
    # real-world failure frequency), this variant picks uniformly from the
    # full taxonomy so each sub-pattern contributes ~1/25 of the dataset.
    # Prerequisite-dependent patterns that return None are re-rolled.
    tried: set = set()
    for _ in range(len(ALL_EQUAL_SHARE_PATTERNS) * 2):
        candidates_list = [p for p in ALL_EQUAL_SHARE_PATTERNS if p not in tried]
        ...
        if mode == 'inline':
            result = generate_inline_repetition_forced(content, pattern)
            if result is not None:
                return result

        else:  # cf
            if pattern in ('cycling', 'incrementing', 'pure'):
                return _generate_cf_html_fallback(content, pattern, target_length)

            result = _dispatch_cf_prereq_pattern(content, pattern)
            if result is not None:
                rej, name = result
                return rej, f'cf_{name}'
            # prerequisite missing — fall through to try another pattern

    # Last resort: always-applicable HTML fallback.
    fallback = random.choice(['cycling', 'incrementing', 'pure'])
    return _generate_cf_html_fallback(content, fallback, target_length)


# -----------------------------------------------------------------------------
# Equal-share dispatch helpers
# -----------------------------------------------------------------------------

CF_EQUAL_SHARE_PATTERNS: List[str] = [
    'cycling', 'incrementing', 'pure',
    'css_rule_cycling', 'enumeration',
    'whitespace_runaway', 'css_bloat', 'section_repetition',
    'self_closing_spam', 'closing_tag_spam',
    'truncated_padded', 'css_truncated_other',
]  # 12 cf sub-patterns

INLINE_EQUAL_SHARE_PATTERNS: List[str] = INLINE_HTML_TYPES + INLINE_CSS_TYPES  # 13

ALL_EQUAL_SHARE_PATTERNS: List[Tuple[str, str]] = (
    [('cf', p) for p in CF_EQUAL_SHARE_PATTERNS]
    + [('inline', p) for p in INLINE_EQUAL_SHARE_PATTERNS]
)  # 25 total, each sampled with probability 1/25


def _dispatch_cf_prereq_pattern(content: str, pattern: str) -> Optional[Tuple[str, str]]:
    """Dispatch to a prerequisite-dependent cf_* generator; return None if
    the chosen pattern cannot be produced from this content."""
    if pattern == 'css_rule_cycling':
        return generate_css_rule_cycling(content)
    if pattern == 'enumeration':
        return generate_enumeration_loop(content)
    if pattern == 'whitespace_runaway':
        return generate_whitespace_runaway(content)
    if pattern == 'css_bloat':
        return generate_css_bloat(content, force_low_volume=False)
    if pattern == 'css_truncated_other':
        # Force the low-volume branch of generate_css_bloat — this is the
        # "cf_css_truncated_other" sub-pattern, which the frequency-weighted
        # dispatcher can only reach stochastically (30% of css_bloat calls).
        return generate_css_bloat(content, force_low_volume=True)
    if pattern == 'section_repetition':
        return generate_section_repetition(content)
    if pattern == 'self_closing_spam':
        return generate_self_closing_spam(content)
    if pattern == 'closing_tag_spam':
        return generate_closing_tag_spam(content)
    if pattern == 'truncated_padded':
        return generate_truncated_padded(content)
    return None


def _generate_cf_html_fallback(content: str, sub_pattern: str, target_length: int) -> Tuple[str, str]:
    """Always-applicable HTML completion-failure generator for cycling /
    incrementing / pure patterns. Extracted verbatim from the original
    dispatcher."""
    ...
    full = prefix + suffix
    return full[:target_length], f'cf_{sub_pattern}'
...
def main():
    parser = argparse.ArgumentParser(
        description="Transform ms_swift JSONL dataset to DPO format with infinite-nesting rejected samples (equal-share distribution across all 25 sub-patterns)"
    )
    ...
    print("\n" + "=" * 60)
    print("TRANSFORMATION STATISTICS (INFINITE NESTING + INLINE — EQUAL SHARE)")
    print("=" * 60)
