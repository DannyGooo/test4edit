#!/bin/bash
#
# Transform ms_swift JSONL dataset to DPO format with infinite-nesting rejected samples
#
# Usage:
#   ./run_transform_dpo_infinite_nesting.sh --input <input.jsonl> --output <output.jsonl> [--limit N] [--seed S]
#
...
OUTPUT="/home/liu282/scratch3/projects/vision_to_code/dataset_clean/htmlSlicer/output/screenshot_final_outout/ms_swift/meta_data_web_clean_8000_1280_12800_human_read_token8000_dpo_infinite_nesting.jsonl"
LIMIT=""
SEED="42"
...
usage() {
    ...
    echo "Qwen3-VL/Qwen2.5-VL inference output."
    echo ""
...
echo -e "${BLUE}===================================================${NC}"
echo -e "${BLUE}ms_swift DPO Transformation (Infinite Nesting)${NC}"
echo -e "${BLUE}===================================================${NC}"
echo ""
...
# Build command
CMD="python3 ${SCRIPT_DIR}/transform_to_dpo_infinite_nesting.py --input \"$INPUT\" --output \"$OUTPUT\" --seed $SEED"
