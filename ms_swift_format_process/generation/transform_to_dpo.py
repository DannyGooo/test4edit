#!/usr/bin/env python3
"""
Transform ms_swift JSONL dataset to DPO format with synthetic loopy rejected samples.
...
- response: Original ground truth HTML/CSS (chosen)
- rejected_response: Synthetic "loopy" generation that simulates model getting stuck in repetition

Usage:
  python3 transform_to_dpo.py --input data.jsonl --output data_dpo.jsonl [--limit N] [--seed S]
"""
...
# (reused from qwen_series_format_process/transform_to_dpo_random_repetition)
# =============================================================================
...
def get_leading_indent(content: str, pos: int) -> str:
    ...
    if line_start == -1:
        # pos is on the very first line
        indent = content[:pos]
        # Only keep if it's pure whitespace
        if indent and indent.strip() == '':
            return indent
        return ''
    # Characters between the newline and the first non-space on that line
...
def generate_incrementing_tag_loop(content: str, start_pos: int, target_length: int) -> str:
    ...
    indent = get_leading_indent(content, start_pos)
    sample_tag = f'{indent}<{tag_name} class="{class_name}-1">'
...
def generate_css_incrementing_loop(content: str, start_pos: int, target_length: int) -> str:
    ...
    indent = get_leading_indent(content, start_pos)
    sample_rule = f'{indent}{base_selector}-1 {{ }}\n'
...
def main():
    parser = argparse.ArgumentParser(
        description="Transform ms_swift JSONL dataset to DPO format with loopy rejected samples"
    )
    parser.add_argument(
        "--input", "-i",
        required=True,
