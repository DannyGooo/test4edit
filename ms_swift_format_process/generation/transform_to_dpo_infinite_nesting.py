...
All 18 sub-patterns are length-matched: len(rejected) == len(chosen). Mode A vs
Mode B is chosen with equal weight per entry.
...
def generate_infinite_nesting_rejected(content: str) -> Tuple[str, str]:
    ...
    full = prefix + suffix
    # Trim to exactly target_length (truncates last line, simulating max_token cutoff)
...
def main():
    parser = argparse.ArgumentParser(
        description="Transform ms_swift JSONL dataset to DPO format with infinite-nesting rejected samples"
    )
    ...
    print("\n" + "=" * 60)
    print("TRANSFORMATION STATISTICS (INFINITE NESTING + INLINE)")
    print("=" * 60)
