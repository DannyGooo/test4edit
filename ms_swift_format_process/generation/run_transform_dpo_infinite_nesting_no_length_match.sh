...
# samples. This variant does NOT guarantee len(rejected) == len(chosen);
# rejected length is drawn from a random fraction of the chosen length, matching
# the variance profile of run_transform_dpo.sh while keeping the rich 25-pattern
# failure taxonomy of the infinite-nesting generator.
#
# Usage:
#   ./run_transform_dpo_infinite_nesting_no_length_match.sh --input <input.jsonl> --output <output.jsonl> [--limit N] [--seed S]
#
...
OUTPUT="/home/liu282/scratch3/projects/vision_to_code/dataset_clean/htmlSlicer/output/screenshot_final_outout/ms_swift/meta_data_web_clean_8000_1280_12800_human_read_token8000_dpo_infinite_nesting_no_length_match.jsonl"
LIMIT=""
SEED="42"

# Function to display usage
usage() {
    echo -e "${BLUE}ms_swift DPO Dataset Transformation Tool (Infinite Nesting, NO Length Match)${NC}"
    echo ""
    echo "Transforms ms_swift JSONL dataset to DPO format with length-variable"
    echo "rejected samples covering 25 failure sub-patterns observed in real"
    echo "Qwen3-VL/Qwen2.5-VL inference output. Unlike the length-matched sibling"
    echo "script, len(rejected) is NOT forced to equal len(chosen)."
    echo ""
    echo -e "${YELLOW}Coverage (50/50 mode dispatch):${NC}"
...
echo -e "${BLUE}=========================================================${NC}"
echo -e "${BLUE}ms_swift DPO Transformation (Infinite Nesting, NO Match)${NC}"
echo -e "${BLUE}=========================================================${NC}"
echo ""
...
# Build command
CMD="python3 ${SCRIPT_DIR}/transform_to_dpo_infinite_nesting_no_length_match.py --input \"$INPUT\" --output \"$OUTPUT\" --seed $SEED"
